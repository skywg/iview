...
public class AppUtil {
	...
	public static boolean checkParam(String method, PageData pd) {
		...
		if (method == "login_login") {
			paramArray = new String[] { "USERNAME", "PASSWORD", "CODE" };
			valueArray = new String[] { "用户名", "密码", "验证码" };
		}
		int size = paramArray.length;
		for (int i = 0; i < size; i++) {
			String param = paramArray[i];
			if (!pd.containsKey(param) || pd.getString(param).equals("null") || pd.getString(param).equals("")) {
				tempArray[falseCount] = valueArray[i] + "--" + param;
				falseCount += 1;
			}
		}
		if (falseCount > 0) {
			logger.error(method + "接口，请求协议中缺少 " + falseCount + "个 参数或参数值为空");
